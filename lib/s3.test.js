...
describe("s3", () => {
  const rec = {
    ...
    key: "the/path/foo.bar",
  };

  describe("#path", () => {
    it("returns the S3 path for a recording", () => {
      expect(path(rec)).toEqual("the/path");

      process.env.S3_PREFIX = "some/thing";
      expect(path(rec)).toEqual("some/thing/the/path");
    });
  });

  describe("#key", () => {
    it("returns the S3 key for a recording", () => {
      expect(key(rec)).toEqual("the/path/foo.bar");

      process.env.S3_PREFIX = "some/thing";
      expect(key(rec)).toEqual("some/thing/the/path/foo.bar");
    });
  });

  describe("#wip", () => {
    it("returns the S3 key for a wip", () => {
      expect(wip(rec)).toEqual("the/path/foo.bar.wip");

      process.env.S3_PREFIX = "some/thing";
      expect(wip(rec)).toEqual("some/thing/the/path/foo.bar.wip");
    });
  });
  ...
  describe("getProgress", () => {
    it("looks for s3 recording in progress files", async () => {
      mockS3List((params) => {
        ...
        expect(params.Prefix).toEqual("the/path");
        return { Contents: [] };
      });
  ...
  describe("putProgress", () => {
    it("writes an s3 progress file", async () => {
      mockS3Put((params) => {
        expect(params.Bucket).toEqual("my-bucket");
        expect(params.Key).toEqual("some/thing/the/path/foo.bar.wip");
