...
import { v4 } from "uuid";
import { timezoneDay, timezoneHours } from "./timezone.js";
...
export function parse(cfgs = [], now = new Date()) {
  return cfgs.flatMap((cfg) => {
    return recordTimes(cfg, now).filter((rec) => {
      ...
      const day = timezoneDay(rec.hour, rec.timeZone);
      if (rec.recordDays?.length && !rec.recordDays.includes(day)) {
        return false;
      }

      const hours = timezoneHours(rec.hour, rec.timeZone);
      if (rec.recordHours?.length && !rec.recordHours.includes(hours)) {
        return false;
      }
