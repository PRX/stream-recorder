...
export function parse(cfgs = [], now = new Date()) {
  return cfgs.flatMap((cfg) => {
    return recordTimes(cfg, now).filter((rec) => {
      const tz = rec.timeZone || null;
      if (tz && !Intl.supportedValuesOf("timeZone").includes(tz)) {
        log.warn("Unsupported timezone", { ...rec });
      }
      process.env.TZ = tz;
      ...
      const day = tz ? rec.hour.getDay() : rec.hour.getUTCDay();
      if (rec.recordDays?.length && !rec.recordDays.includes(day)) {
        return false;
      }

      const hour = tz ? rec.hour.getHours() : rec.hour.getUTCHours();
      if (rec.recordHours?.length && !rec.recordHours.includes(hour)) {
        return false;
      }
