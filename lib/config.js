...
export function parse(cfgs = [], now = new Date()) {
  return cfgs.flatMap((cfg) => {
    return recordTimes(cfg, now).filter((time) => {
      if (time.startDate) {
        const startDate = new Date(`${time.startDate}T00:00:00Z`);
        if (time.hour < startDate) {
          return false;
        }
      }

      if (time.endDate) {
        const endDate = new Date(`${time.endDate}T00:00:00Z`);
        endDate.setUTCDate(endDate.getUTCDate() + 1);
        // console.log(`${time.hour.toISOString()} >= ${endDate.toISOString()}`, time.hour >= endDate);
        if (time.hour >= endDate) {
          return false;
        }
      }

      if (time.recordDays?.length && !time.recordDays.includes(time.hour.getUTCDay())) {
        return false;
      }

      if (time.recordHours?.length && !time.recordHours.includes(time.hour.getUTCHours())) {
        return false;
      }
