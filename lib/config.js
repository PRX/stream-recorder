...
export function recordTimes(cfg = {}, now = new Date()) {
  return hours(now).map((hour) => {
    ...
    // replace date/hour in the job id
    const str = hour.toISOString();
    const dateStr = str.substr(0, 10);
    const hourStr = str.substr(11, 2);
    const jobId = (cfg.job_id || uuid).replace(":date", dateStr).replace(":hour", hourStr);
    ...
    return { ...camelcase(cfg), jobId, hour, start, stop, filename };
  });
}
