...
describe("config", () => {
  describe("#parse", () => {
    ...
    it("checks end dates", () => {
      ...
      // but it COULD stop recording after endDdate due to a buffer
      process.env.BUFFER_END = 121;
      expect(parse([{ endDate: "2025-12-08" }], new Date("2025-12-09T00:03Z"))).toHaveLength(0);
      expect(parse([{ endDate: "2025-12-08" }], new Date("2025-12-09T00:02Z"))).toHaveLength(1);
    });
  ...
  describe("#recordTimes", () => {
    it("combines configs with their hour start/stop times", () => {
      ...
      const times = recordTimes({ what_ev: "val" }, new Date("2025-12-09T21:58Z"));
      expect(times.length).toEqual(2);
      expect(times[0]).toEqual({
        whatEv: "val",
        hour: new Date("2025-12-09T21:00:00.000Z"),
        start: new Date("2025-12-09T20:58:00.000Z"),
        stop: new Date("2025-12-09T21:00:05.000Z"),
      });
      expect(times[1]).toEqual({
        whatEv: "val",
        hour: new Date("2025-12-09T22:00:00.000Z"),
        start: new Date("2025-12-09T21:58:00.000Z"),
        stop: new Date("2025-12-09T22:00:05.000Z"),
      });
    });
  });
  ...
  describe("#hours", () => {
    ...
    it("applies a start time buffer", () => {
      ...
      expect(hours(new Date("2025-12-09T21:59:59Z"))).toEqual([h1, h2]);
      expect(hours(new Date("2025-12-09T22:00Z"))).toEqual([h2]);
    });

    it("applies an end time buffer", () => {
      const h1 = new Date("2025-12-09T21:00Z");
      const h2 = new Date("2025-12-09T22:00Z");

      process.env.BUFFER_START = 1;
      process.env.BUFFER_END = 120;
      expect(hours(new Date("2025-12-09T21:59:59Z"))).toEqual([h1, h2]);
      expect(hours(new Date("2025-12-09T22:00Z"))).toEqual([h1, h2]);
      expect(hours(new Date("2025-12-09T22:01:59.999Z"))).toEqual([h1, h2]);
      expect(hours(new Date("2025-12-09T22:02:00Z"))).toEqual([h2]);
    });
  });
});
